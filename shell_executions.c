...
/**
 * handle_commands - Manages command execution and control flow using logical
 * operators
...
int handle_commands(char **args)
{
	...
	if (no_error == FALSE || command_result == FALSE)
		return (FALSE);
...
int built_ins(char **args)
{
	...
	else if (compare_strings("setenv", *args, MATCH) == TRUE &&
		 args[1] != NULL)
	{
...
int and_or(char **args, char operator, int last_compare)
{
	...
	else if (last_compare == TRUE && operator == '&')
	{
		result = execute_command(args);
	}
	else if (last_compare == FALSE && operator == '|')
	{
		result = execute_command(args);
	}
	else if (last_compare == TRUE && operator == '|')
...
char *find_executable_path(char **args)
{
	...
	while (token != NULL)
	{
		char *full_path = (char *)malloc(strlen(token) + strlen(command_name) + 2);
